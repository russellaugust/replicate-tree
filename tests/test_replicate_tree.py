...
def test_parse_arguments():
    ...
    with patch("sys.argv", ["replicate", "-s", source, "-d", destination]):
        args = replicate_tree.parse_arguments()
    ...
    with patch("sys.argv", ["replicate", "-s", source]), pytest.raises(SystemExit):
        args = replicate_tree.parse_arguments()
...
def test_touch_and_ffprobe_videofile(tmp_path: Path):
    ...
    replicate_tree.touch_and_ffprobe(src_file, dst_file, True)

    assert dst_file.stat().st_size > 0


def test_supported_formats_from_ffmpeg():
    result = replicate_tree.supported_input_formats()
    assert isinstance(result, list)


def test_replicate_directories_files_that_are_never_touched_or_created(tmp_path: Path):
    with patch("replicate_tree.touch_and_ffprobe", return_value=None):
        src_base = Path("tests/tree")
        dst_base = tmp_path / "FAKE_RAID2"

        replicate_tree.process_directory(src_base, dst_base)
...
#     replicate_tree.process_directory(src_base, dst_base, metadata=True)
...
def test_replicate_directories_and_store_dummy_data(tmp_path: Path):
    ...
    replicate_tree.process_directory(src_base, dst_base, files=False)
...
def test_extract_media_metadata_nonzero_exit_ffprobe():
    src_file = Path("tests/tree/bad-files/global.d.ts")
    result = replicate_tree.extract_media_metadata(src_file)
...
def test_extract_image_metadata():
    ...
    for src_file in folder_path.iterdir():
        ...
        if src_file.is_file():
            result = replicate_tree.extract_image_metadata(src_file)
            assert isinstance(result, dict)
...
#     ffmpeg_installed = replicate_tree.check_installation("ffmpeg")
#     assert isinstance(ffmpeg_installed, bool)

#     with patch('replicate_tree.subprocess.run', return_value="ffprobe"):
#         ffprobe_installed = replicate_tree.check_installation("ffprobe")
